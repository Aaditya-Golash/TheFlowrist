...
const ORDER_FIELDS = {
  ...
  occasionType: 'occasion_type',
  occasionLabel: 'occasion_label',
  surpriseSettingId: 'surprise_setting_id',
  budgetTier: 'budget_tier',
...
const MEMBERSHIP_FIELDS = {
  ...
  planKey: 'plan_key',
  status: 'status',
  annualFeeCents: 'annual_fee_cents',
  protectedDateLimit: 'protected_date_limit',
...
const SURPRISE_SETTING_FIELDS = {
  id: 'id',
  userId: 'customer_id',
  recipientId: 'recipient_id',
  budgetTier: 'budget_tier',
  monthlyPriceCents: 'monthly_price_cents',
  preferredDeliveryDay: 'preferred_delivery_day',
  preferredDeliveryDate: 'preferred_delivery_date',
  reminderDaysBefore: 'reminder_days_before',
  chargeDaysBefore: 'charge_days_before',
  status: 'status',
  skippedMonth: 'skipped_month',
  lastGeneratedMonth: 'last_generated_month',
  notes: 'notes',
  createdAt: 'created_at',
  updatedAt: 'updated_at',
};
...
function createSupabaseStore(env = process.env, { createClientImpl = createClient } = {}) {
  ...
  async function query(table, options = {}) {
    ...
    if (error) {
      if (options.optionalMissingTable && (error.code === 'PGRST205' || /Could not find the table/i.test(error.message || ''))) {
        return [];
      }
      throw error;
    }
  ...
  return {
    async getState() {
      const [customers, recipients, milestones, orders, floristPartners, serviceZones, paymentConsents, orderEvents, feedback, relationshipMemberships, surpriseDelightSettings] = await Promise.all([
        query('customers'),
        query('recipients'),
        query('milestones'),
        ...
        query('feedback'),
        query('relationship_memberships', { optionalMissingTable: true }),
        query('surprise_delight_settings', { optionalMissingTable: true }),
      ]);
      ...
      return {
        ...
        relationshipMemberships: relationshipMemberships.map((row) => toCamel(row, MEMBERSHIP_FIELDS)),
        surpriseDelightSettings: surpriseDelightSettings.map((row) => toCamel(row, SURPRISE_SETTING_FIELDS)),
      };
    },
    async saveState() {
    ...
    async listRelationshipMemberships() {
      const rows = await query('relationship_memberships', { optionalMissingTable: true });
      return rows.map((row) => toCamel(row, MEMBERSHIP_FIELDS));
    },
    async createRelationshipMembership(membership) {
      const row = await insert('relationship_memberships', toSnake(membership, MEMBERSHIP_FIELDS));
      return toCamel(row, MEMBERSHIP_FIELDS);
    },
    async updateRelationshipMembership(membershipId, changes) {
      const row = await update('relationship_memberships', membershipId, toSnake({ ...changes, updatedAt: new Date().toISOString() }, MEMBERSHIP_FIELDS));
      return toCamel(row, MEMBERSHIP_FIELDS);
    },
    async listSurpriseDelightSettings() {
      const rows = await query('surprise_delight_settings', { optionalMissingTable: true });
      return rows.map((row) => toCamel(row, SURPRISE_SETTING_FIELDS));
    },
    async createSurpriseDelightSetting(setting) {
      const row = await insert('surprise_delight_settings', toSnake(setting, SURPRISE_SETTING_FIELDS));
      return toCamel(row, SURPRISE_SETTING_FIELDS);
    },
    async updateSurpriseDelightSetting(settingId, changes) {
      const row = await update('surprise_delight_settings', settingId, toSnake({ ...changes, updatedAt: new Date().toISOString() }, SURPRISE_SETTING_FIELDS));
      return toCamel(row, SURPRISE_SETTING_FIELDS);
    },

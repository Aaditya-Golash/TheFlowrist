...
const { getSurpriseMonthlyTier } = require('./plans');
...
function addMonths(dateValue, months) {
  const parsedDate = new Date(`${dateValue}T12:00:00`);
  parsedDate.setMonth(parsedDate.getMonth() + Number(months || 0));
  return parsedDate.toISOString().slice(0, 10);
}

function isAtLeastDaysOut(dateValue, daysOut) {
  const parsedDate = new Date(`${dateValue}T12:00:00`);
  const minimumDate = new Date();
  minimumDate.setHours(0, 0, 0, 0);
  minimumDate.setDate(minimumDate.getDate() + Number(daysOut || 0));
...
function createScheduledOrderFromMilestone(milestone, user, serviceZone) {
  ...
  return {
    ...
    plannedChargeDate,
    reminderDate: calculateReminderDate(milestone.eventDate, milestone.reminderDaysBefore || 7),
    orderSource: 'milestone',
    occasionType: milestone.occasionType,
    occasionLabel: milestone.occasionLabel || '',
    surpriseSettingId: null,
...
function createOneTimeScheduledOrder(values, user, recipient, serviceZone) {
  const eventDate = values.eventDate;
  const chargeDaysBefore = Number(values.chargeDaysBefore || 5);
  const reminderDaysBefore = Number(values.reminderDaysBefore || 7);
  const deliveryFee = serviceZone?.deliveryFeeCents || 0;
  return {
    id: `order-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`,
    userId: user.id,
    ...
    milestoneId: null,
    eventDate,
    plannedChargeDate: calculatePlannedChargeDate(eventDate, chargeDaysBefore),
    reminderDate: calculateReminderDate(eventDate, reminderDaysBefore),
    orderSource: 'one_time',
    occasionType: values.occasionType || 'custom',
    occasionLabel: values.occasionLabel || '',
    surpriseSettingId: null,
    budgetTier: values.budgetTier || 'classic',
    estimatedCustomerPriceCents: calculateEstimatedPrice(values.budgetTier || 'classic', deliveryFee),
    deliveryFeeCents: deliveryFee,
    status: 'scheduled',
    floristPartnerId: null,
    internalNotes: '',
    customerNotes: values.customerNotes || '',
    generatedCardMessage: values.cardMessageTone ? `A thoughtful note for ${values.cardMessageTone}.` : '',
    photoProofUrl: '',
    deliveredAt: null,
    supportMinutes: 0,
    refundAmountCents: null,
...
function createSurpriseScheduledOrder(setting, user, recipient, targetMonth, serviceZone) {
  const eventDate = setting.preferredDeliveryDate || `${targetMonth}-15`;
  const reminderDaysBefore = Number(setting.reminderDaysBefore || 7);
  const chargeDaysBefore = Number(setting.chargeDaysBefore || 5);
  const tier = getSurpriseMonthlyTier(setting.budgetTier);
  const deliveryFee = serviceZone?.deliveryFeeCents || 0;
  return {
    id: `order-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`,
    userId: user.id,
    ...
    plannedChargeDate: calculatePlannedChargeDate(eventDate, chargeDaysBefore),
    reminderDate: calculateReminderDate(eventDate, reminderDaysBefore),
    orderSource: 'surprise_monthly',
    occasionType: 'surprise_delight',
    occasionLabel: 'Surprise & Delight',
    surpriseSettingId: setting.id,
    budgetTier: setting.budgetTier || 'classic',
    estimatedCustomerPriceCents: setting.monthlyPriceCents || tier.monthlyPriceCents,
    deliveryFeeCents: deliveryFee,
    status: 'scheduled',
    floristPartnerId: null,
    internalNotes: `Generated from Surprise & Delight setting for ${targetMonth}.`,
    customerNotes: setting.notes || '',
    generatedCardMessage: 'A thoughtful note for a spontaneous seasonal gesture.',
    photoProofUrl: '',
    deliveredAt: null,
    supportMinutes: 0,
    refundAmountCents: null,
...
function getOrdersNeedingReminder(orders, milestones) {
  ...
  return orders.filter((order) => {
    ...
    if (order.reminderDate) {
      return order.reminderDate <= today;
    }

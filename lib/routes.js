...
function tooManyAttemptsResponse(res, title) {
  renderNoticePage(res, 429, {
    title,
    message: 'Too many attempts. Please wait a few minutes and try again â€” your progress remains safe.',
  });
}
...
async function handleRequest(req, res) {
  {
    ...
    if (req.method === 'GET' && pathname === '/') {
      ...
      const body = `
        <section class="hero">
          <div>
            <div class="pill">Milestone flower concierge Â· Toronto private pilot</div>
            <h1 class="hero-title">Never forget flowers for the dates that matter.</h1>
            <p class="muted">TheFlowrist isn't a flower shop and it isn't a weekly subscription. Tell us the dates that matter â€” birthdays, anniversaries, thank-yous â€” and our concierge team quietly takes care of the rest, with a reminder before anything is ever charged.</p>
          ...
          <div class="panel">
            ...
            <ul>
              ...
              <li>A reminder before any charge â€” never a surprise</li>
        ...
        <section class="section panel" id="how-it-works">
          ...
          <ol class="steps">
            <li><div><strong>Tell us who and when</strong><span>Add a recipient and the date you want to protect â€” a birthday, anniversary, or any occasion that matters.</span></div></li>
            <li><div><strong>Choose a budget tier</strong><span>Classic, Premium, or Signature â€” no need to pick a specific bouquet.</span></div></li>
            ...
            <li><div><strong>Pause or cancel anytime</strong><span>Change your mind before the cutoff and nothing happens â€” no questions asked.</span></div></li>
          </ol>
        </section>
        ...
        <section class="section">
          ...
          <div class="trust-grid">
            <div class="trust-item"><h4>No weekly subscription</h4><p>You only get flowers on the dates you choose to protect â€” nothing recurring, nothing you didn't ask for.</p></div>
            ...
            <div class="trust-item"><h4>Designer's-choice seasonal flowers</h4><p>Our florist partners choose the best seasonal stems for the moment â€” this keeps quality high and never overpromises an exact bouquet.</p></div>
            <div class="trust-item"><h4>Toronto private pilot</h4><p>We're intentionally starting small â€” a private concierge pilot for Toronto and nearby service zones.</p></div>
          </div>
        </section>

        <section class="section">
          <h2 class="section-title">Budget tiers</h2>
          <p class="section-lead">Choose a tier, not a specific arrangement â€” our florist partner handles the rest.</p>
          <div class="tier-grid">
        ...
        <section class="section faq panel">
          ...
          <details>
            ...
            <p>No. You're never charged the day you add a date. We send a reminder first, then only charge close to the delivery date â€” and you can pause or cancel before that.</p>
          </details>
          <details>
            <summary>Can I pause or cancel?</summary>
            <p>Yes, anytime before the charge cutoff. Pause a date temporarily or cancel it entirely from your dashboard â€” no phone calls needed.</p>
          </details>
          <details>
          ...
          <details>
            ...
            <p>Our concierge team steps in personally â€” rescheduling, substituting a comparable seasonal arrangement, or refunding, whichever makes sense for the moment.</p>
          </details>
        </section>`;
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
      res.end(renderHtml('TheFlowrist â€” Milestone flower concierge', body, state));
      return;
    }
        ...
        return `
          <div class="milestone-row">
            <div class="milestone-meta">
              ...
              <p>Protected date: ${escapeHtml(milestone.eventDate)} Â· Budget tier: ${escapeHtml(formatStatusLabel(milestone.budgetTier))}</p>
      ...
      const body = `
        ...
        <section class="section card">
          ...
          ${protectedMilestones.length ? milestoneRows : `
            ...
            <p class="muted">Start with one birthday or anniversary â€” it takes about a minute.</p>
            <a class="btn" href="/milestones/new">Add your first important date</a>
          `}
        </section>
        <section class="section card">
          <h2 class="section-title">Recipients</h2>
          ${ownRecipients.length ? ownRecipients.map((recipient) => `<p><strong>${escapeHtml(recipient.name)}</strong> Â· ${escapeHtml(recipient.relationship)}<br/>${escapeHtml(recipient.city)} Â· ${escapeHtml(recipient.postalCode)}</p>`).join('') : '<p class="muted">Add your first recipient to start gifting thoughtfully.</p>'}
        </section>`;
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
    ...
    if (req.method === 'GET' && pathname === '/recipients/new') {
      ...
      const body = `
        <section class="section form-shell">
          ...
          <p class="section-lead">Save their details once â€” you'll reuse this recipient for every future milestone. <span class="helper helper-inline">You can edit this later.</span></p>
          <form action="/recipients" method="post">
            <label>Name<input name="name" required /></label>
            <label>Relationship<input name="relationship" placeholder="Sister, mom, best friendâ€¦" required /><span class="helper">However you'd describe them â€” this just helps our concierge team personalize things.</span></label>
    ...
    if (req.method === 'GET' && pathname === '/milestones/new') {
      ...
      const body = `
        <section class="section form-shell">
          ...
          <p class="section-lead">Once saved, we'll remind you before anything is charged â€” you're never surprised. <span class="helper helper-inline">You can edit this later.</span></p>
          <form action="/milestones" method="post">
            ...
            </select><span class="helper">A budget range, not a specific bouquet â€” our florist partner curates a Designer's Choice arrangement to fit it.</span></label>
            ...
            <label>Style preferences<textarea name="stylePreferences" placeholder="Soft pastels, bright colors, all-whiteâ€¦"></textarea><span class="helper">Optional. Designer's Choice lets the florist use the best seasonal stems available â€” this just points them in the right direction.</span></label>
            <div class="mood-grid" aria-label="Aesthetic mood examples">
              <label class="mood-card"><input type="radio" name="stylePreferences" value="Pastel & Eucalyptus" /><span class="mood-card-body"><h4>Pastel & Eucalyptus</h4><p>Soft, airy, quietly romantic.</p></span></label>
              <label class="mood-card"><input type="radio" name="stylePreferences" value="Sculptural & Clean" /><span class="mood-card-body"><h4>Sculptural & Clean</h4><p>Minimal, architectural, restrained.</p></span></label>
              <label class="mood-card"><input type="radio" name="stylePreferences" value="Rich & Botanical" /><span class="mood-card-body"><h4>Rich & Botanical</h4><p>Layered foliage and deeper seasonal color.</p></span></label>
            </div>
          ...
          <p class="muted form-footnote">We'll remind you before anything is charged, and you can pause or cancel from your dashboard before the cutoff.</p>
          </div>
        </section>`;
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
    ...
    if (req.method === 'GET' && pathname.startsWith('/admin/orders/')) {
      ...
      const timeline = (state.orderEvents || []).filter((entry) => entry.orderId === order.id).slice(-8).map((entry) => `<li><strong>${escapeHtml(entry.createdAt)}</strong> Â· ${escapeHtml(entry.actorType)} Â· ${escapeHtml(entry.type)} Â· ${escapeHtml(entry.message)}</li>`).join('');
      const body = `
        <section class="section panel">
          ...
          <div class="summary-list">
            ...
            <p><strong>Recipient:</strong> ${escapeHtml(recipient?.name || 'Unknown')}${recipient ? ` Â· ${escapeHtml(recipient.city)} ${escapeHtml(recipient.postalCode)}` : ''}</p>
    ...
    if (req.method === 'POST' && pathname.startsWith('/admin/orders/')) {
      ...
      if (!validateStatusTransition(order.status, nextStatus)) {
        renderNoticePage(res, 400, { title: 'Invalid status change', message: 'That status change is not allowed from the orderâ€™s current status.' });
        return;
      }
    ...
    if (req.method === 'GET' && pathname === '/account/payment-consent') {
      ...
      const body = `
        <section class="section vault-panel">
          ...
          <p class="muted">This step is not a real charge â€” it records your consent for our concierge team to charge your saved payment method later, only for milestone orders you've explicitly scheduled. Real card charging isn't wired up yet in this pilot.</p>
          ...
          <form action="/account/payment-consent" method="post">
            ...
            <button class="btn btn-primary" type="submit">Save payment consent</button>
          </form>

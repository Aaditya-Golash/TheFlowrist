...
async function handleRequest(req, res) {
  {
      ...
      const body = `
        ...
        <section class="section">
          ...
          <div class="trust-grid">
            ...
            <div class="trust-item"><h4>No catalog guessing</h4><p>There is no bouquet catalog to browse. Choose a budget tier and our florist partner picks the strongest seasonal stems available that week.</p></div>
      ...
      const body = `
        ...
        <section class="section card">
          ...
          ${protectedMilestones.length ? milestoneRows : `
            ...
            <p class="muted">Start with one birthday, anniversary, or thank-you moment.</p>
        ...
        <section class="section form-shell">
          ...
          <form action="/milestones" method="post">
            ...
            <div class="mood-grid" aria-label="Aesthetic mood examples">
              <label class="mood-card"><input type="radio" name="moodPreset" value="Pastel & Eucalyptus" /><span class="mood-card-body"><h4>Pastel & Eucalyptus</h4><p>Muted rose tones, cream stems, soft greenery.</p></span></label>
              <label class="mood-card"><input type="radio" name="moodPreset" value="Sculptural & Clean" /><span class="mood-card-body"><h4>Sculptural & Clean</h4><p>Structural lines, high-contrast foliage, restrained palette.</p></span></label>
              <label class="mood-card"><input type="radio" name="moodPreset" value="Rich & Botanical" /><span class="mood-card-body"><h4>Rich & Botanical</h4><p>Deep seasonal tones, textural greenery, dramatic shape.</p></span></label>
            </div>
            <span class="helper">Pick a mood card as a starting point, or write your own style notes above &mdash; your own words take priority if you fill in both.</span>
    ...
    if (req.method === 'POST' && pathname === '/milestones') {
      ...
      const freeformStylePreferences = (normalizedValues.stylePreferences || '').trim();
      const moodPreset = (normalizedValues.moodPreset || '').trim();
      const milestone = {
        ...
        stylePreferences: freeformStylePreferences || moodPreset,
        ...
        <section class="section vault-panel">
          ...
          <div class="reassurance">
            ...
            <ul>
              ...
              <li>Payment capture is not enabled in this local pilot build.</li>
            </ul>
          </div>
          <p class="muted">This is not a charge. It records consent for a future concierge payment workflow.</p>
          <p class="muted">Payment details remain server-side and are never shown in this interface.</p>
          <p class="muted">${escapeHtml(consent?.consentTextSnapshot || 'By saving payment consent, you authorize TheFlowerist to charge scheduled flower orders, delivery, and applicable taxes. We remind you before scheduled charges. You can pause or cancel before the cutoff.')}</p>
    ...
    if (req.method === 'POST' && pathname === '/account/payment-consent') {
      ...
      const consent = {
        ...
        consentTextSnapshot: 'By saving payment consent, you authorize TheFlowerist to charge scheduled flower orders, delivery, and applicable taxes. We remind you before scheduled charges. You can pause or cancel before the cutoff.',

...
async function handleRequest(req, res) {
  {
    ...
    if (req.method === 'GET' && pathname === '/dashboard') {
      ...
      const state = getState();
      const protectedMilestones = state.milestones.filter((milestone) => milestone.status !== 'cancelled');
      const milestoneRows = protectedMilestones.map((milestone) => {
        const recipient = state.recipients.find((entry) => entry.id === milestone.recipientId);
        const order = state.orders.find((entry) => entry.milestoneId === milestone.id);

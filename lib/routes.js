...
const { isAdminRequest, getAdminCookieValue, getAdminEmailFromRequest } = require('./admin');
const {
  ...
  return (req, res) => {
    ...
    if (req.method === 'POST' && pathname === '/admin/login') {
      ...
      req.on('end', () => {
        ...
        if (!isAdminRequest({ headers: { 'x-admin-email': email } })) {
    ...
    if (req.method === 'POST' && pathname === '/admin/logout') {
      res.writeHead(302, { Location: '/admin/login', 'Set-Cookie': 'adminEmail=; HttpOnly; Path=/; Max-Age=0; SameSite=Lax' });
      res.end();
      return;
    }

...
const { getStorageAdapter } = require('../store');
...
function createSupabaseAuthAdapter(env = process.env, { createClient: createClientImpl = createClient } = {}) {
  ...
  async function resolveCustomerForUser(authUser) {
    const adapter = getStorageAdapter();
    const customers = await adapter.listCustomers();
    let customer = customers.find((entry) => entry.email && entry.email.toLowerCase() === authUser.email.toLowerCase());
    if (customer && authUser.id && customer.authUserId !== authUser.id) {
      customer = await adapter.updateCustomer(customer.id, { authUserId: authUser.id }) || customer;
    }
    if (!customer) {
  ...
  async function requireUser(req, res) {
    ...
    return await resolveCustomerForUser(authUser);
  }

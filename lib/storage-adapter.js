...
function createJsonStorageAdapter() {
  return {
    ...
    updateCustomer(customerId, changes) {
      ...
      Object.assign(customer, changes, { updatedAt: new Date().toISOString() });
      this.saveState(state);
      return customer;
    },
    listRecipients() {
      return this.getState().recipients || [];
    },
    getRecipientById(id) {
      return this.listRecipients().find((entry) => entry.id === id) || null;
    },
    ...
    getMilestoneById(id) {
      return this.listMilestones().find((entry) => entry.id === id) || null;
    },
    ...
    listRelationshipMemberships() {
      return this.getState().relationshipMemberships || [];
    },
    createRelationshipMembership(membership) {
      const state = this.getState();
      state.relationshipMemberships = state.relationshipMemberships || [];
      state.relationshipMemberships.push(membership);
      this.saveState(state);
      return membership;
    },
    updateRelationshipMembership(membershipId, changes) {
      const state = this.getState();
      state.relationshipMemberships = state.relationshipMemberships || [];
      const membership = state.relationshipMemberships.find((entry) => entry.id === membershipId);
      if (!membership) {
        return null;
      }
      Object.assign(membership, changes, { updatedAt: new Date().toISOString() });
      this.saveState(state);
      return membership;
    },
    listSurpriseDelightSettings() {
      return this.getState().surpriseDelightSettings || [];
    },
    createSurpriseDelightSetting(setting) {
      const state = this.getState();
      state.surpriseDelightSettings = state.surpriseDelightSettings || [];
      state.surpriseDelightSettings.push(setting);
      this.saveState(state);
      return setting;
    },
    updateSurpriseDelightSetting(settingId, changes) {
      const state = this.getState();
      state.surpriseDelightSettings = state.surpriseDelightSettings || [];
      const setting = state.surpriseDelightSettings.find((entry) => entry.id === settingId);
      if (!setting) {
        return null;
      }
      Object.assign(setting, changes, { updatedAt: new Date().toISOString() });
      this.saveState(state);
      return setting;
    },
    ...
    listPaymentConsents() {
      return this.getState().paymentConsents || [];
    },

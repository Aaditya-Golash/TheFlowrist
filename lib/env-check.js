...
function checkStripeEnv(env) {
  ...
  const testMode = !present || String(env.STRIPE_SECRET_KEY).startsWith('sk_test_');
  return {
    ok: (!production || present) && testMode,
    requiredInProduction: production,
    present,
    testMode,
    error: production && !present
      ? 'STRIPE_SECRET_KEY is required in production for real payment capture'
      : testMode
        ? undefined
        : 'STRIPE_SECRET_KEY must be a Stripe test-mode key beginning with sk_test_',
  };
}
...
function validateProductionEnvironment(env = process.env) {
  ...
  if (env.STRIPE_SECRET_KEY && !String(env.STRIPE_SECRET_KEY).startsWith('sk_test_')) {
    errors.push('STRIPE_SECRET_KEY must be a Stripe test-mode key beginning with sk_test_');
  }

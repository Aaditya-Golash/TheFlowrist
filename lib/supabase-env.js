...
function getSupabaseAuthConfig(env = process.env) {
  ...
  return {
    supabaseUrl,
    anonKey,
    hasRequiredConfig: Boolean(supabaseUrl && anonKey),
  };
}

function validateSupabaseAuthEnvironment(env = process.env) {
  const { supabaseUrl, anonKey, hasRequiredConfig } = getSupabaseAuthConfig(env);
  if (!hasRequiredConfig) {
    throw new Error('Supabase auth backend requires SUPABASE_URL and SUPABASE_ANON_KEY');
  }
